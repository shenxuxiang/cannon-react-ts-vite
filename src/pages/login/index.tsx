  ...
  render() {
    return (
      <div className={classes.page}>
        <Spin size="large" className={classes.loading} spinning={this.state.spinning} />
        <video className={classes.video_bg} autoPlay loop muted>
          <source src={huangshan} type="video/mp4" />
        </video>
        <div className={classes.page_x} style={{ height: this.state.updatePassword ? 440 : 380 }}>
          ...
          {this.state.updatePassword ? (
            <Form onFinish={this.handleFormFinish} wrapperCol={{ offset: 8, span: 8 }}>
              <FormItem
                name="oldPassword"
                rules={[
              ...
              >
                <Input size="large" type="password" placeholder="请输入旧密码" prefix={<LockOutlined />} />
              </FormItem>
              <FormItem
                name="newPassword"
                rules={[
                ...
                style={{ marginTop: 40 }}
              >
                <Input size="large" type="password" placeholder="请输入新密码" prefix={<LockOutlined />} />
              </FormItem>
              <FormItem
                ...
                style={{ marginTop: 40 }}
              >
                <Input size="large" type="password" placeholder="确认密码" prefix={<LockOutlined />} />
              </FormItem>
              <FormItem style={{ textAlign: 'center', marginTop: 40 }}>
                <Button style={{ width: 120 }} htmlType="submit" type="primary" size="large">
                  确认修改
                </Button>
              </FormItem>
            </Form>
          ) : (
            <Form wrapperCol={{ offset: 8, span: 8 }} onFinish={this.handleFormFinish}>
              <FormItem
                name="username"
                rules={[
                  ...
                  {
                    pattern: /^[a-zA-Z]\w+$/,
                    message: '用户名必须以大写或小写字母开头，可以包含（数字、字母、下划线）',
                  },
                ]}
              >
                <Input size="large" prefix={<UserOutlined />} placeholder="请输入以字母开头的用户名" />
              </FormItem>
              <FormItem
                name="password"
                rules={[
        ...
        <div className={classes.footer}>技术支持单位：安徽阡陌网络科技有限公司</div>
      </div>
    );
  }
}
